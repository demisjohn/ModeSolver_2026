...
WG = Waveguide(clad(3.0) + core_outer(0.5) + core_mid(1.0) + core_mid(0.5) + clad(3.0))

fig_rix, ax_rix = WG.plot_refractive_index_profile(WG)
fig_rix.show()

fileout = f"{fileout}_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S.%f')[:-3]}"
out = Path(__file__).resolve().parent / f"{fileout + ' RIX'}.png"
...
fig.suptitle(
    ...
    "ModeSolver_2026 · EMpy VFDModeSolver",
    fontsize=11,
)


out = Path(__file__).resolve().parent / f"{fileout}.png"
